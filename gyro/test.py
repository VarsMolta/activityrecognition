...
def getModel(feature, numFeatures, numSamples):
		...
		Y = np.array([1*np.ones(numSamples),2*np.ones(numSamples), 3*np.ones(numSamples), 4*np.ones(numSamples)])
		Y = Y.flatten()	
...
def main():
	...
	for x in range (0,17):
		fc2 = getFeatureForTest(walkData, drinkData, typeData, eatData, 6000+x*200, 6000+x*200+400, freqDecimate100 )
