...
'''
def getFeatures(walkData, drinkData, typeData, eatData, tvData, pourData, startSeconds, stopSeconds, freqHz, feature, iteration):
		print startSeconds 
		print stopSeconds
		...
		pourdata = pourData[startSeconds:stopSeconds:freqHz,1:4:].astype(float)
	
		alldata = np.concatenate((wdata, ddata, tdata, edata, tvdata, pourdata), axis=1)
		print alldata
...
def getModel(feature, numFeatures, numSamples):
		...
		for iteration in range(0,numSamples):
			...
			fc6 = feature[iteration*numFeatures:(iteration+1)*numFeatures,15:18:].flatten()
		...
		Y = np.array([1*np.ones(numSamples),2*np.ones(numSamples), 3*np.ones(numSamples), 4*np.ones(numSamples), 5*np.ones(numSamples), 6*np.ones(numSamples)])
		Y = Y.flatten()	

		target_names = np.array(["walking", "drinking", "typing", "eating", "remoteTV", "pouring Coffee"])
		...
		pl.figure()
		for c, i, target_name in zip("rgbykm", [1, 2, 3, 4, 5, 6], target_names):
...
def getFeatureForTest(walkData, drinkData, typeData, eatData, tvData, pourData, startSeconds, stopSeconds, freqHz):
...
def main():
	...
	#pour_filename = "pouring1_accel.txt"
	pour_filename = "driving1_accel.txt"
	pour_rows = readCSV(pour_filename)
	pourData = np.array(pour_rows)
	...
	for iteration in range(0,numSamples): 	
		feature1 = getFeatures(walkData, drinkData, typeData, eatData, tvData, pourData, 100*(startSeconds+overlap*iteration), 100*(startSeconds+numSecondsPerCalc+overlap*iteration), freqDecimate100, feature2,0)
	...
	print "Prediction ([1] = Walking, [2] = Drinking, [3] = Typing, [4] = Eating, [5] = Watching TV with Remote in Hand, [6] Pouring Drink). Change return value output of getFeaturesForTest for testing all activities"
	# this will test the model from 17 seconds to 27 seconds in 4s intervals with 2s overlap
	for x in range (0,20):
		fc2 = getFeatureForTest(walkData, drinkData, typeData, eatData, tvData, pourData, 5000+x*200, 5000+x*200+400, freqDecimate100 )
